...
library(tidyverse)
head(varcomp)
vc=varcomp |> ggplot(aes(x=effect,y=var, fill=trait)) + geom_col(position = "dodge") + theme(axis.text.x = element_text(angle=90), legend.position = "none") + facet_wrap(~trait,scales="free")

ggsave(plot=vc,device = "pdf",filename ="Plots/varcomp.pdf",dpi = "retina",height = 6,width = 8)
ggsave(plot=vc,device = "png",filename ="Plots/varcomp.png",dpi = "retina",height = 6,width = 8,bg = "white")
...
GY=GY+ theme(axis.text.y = element_text(size=3,color = ifelse(GY$data$gen %in% gen.sel,  "#E41A1C", "black"))) + labs( title = "Grain Yield")
PH=PH+ theme(axis.text.y = element_text(size=3, color = ifelse(PH$data$gen %in% gen.sel,  "#E41A1C", "black"))) + labs( title = "Plant Height")
PL=PL+ theme(axis.text.y = element_text(size=3, color = ifelse(PL$data$gen %in% gen.sel,  "#E41A1C", "black"))) + labs( title = "Plant Lodging")

plot.within = ggarrange(GY,PH,PL,labels = c("A","B","C"),ncol=3, common.legend = T)


ggsave(plot=plot.within,device = "pdf",filename ="Plots/plots_perfo_within.pdf",dpi = "retina",height = 6,width = 8)
ggsave(plot=plot.within,device = "png",filename ="Plots/plots_perfo_within.png",dpi = "retina",height = 6,width = 8,bg = "white")
...
,size = 2,shape = 21,stroke = 0.4) +guides(fill = "none") +  
  labs(y = "", x = "", title = "Plant Height")+theme(axis.text.x = element_text(size = 5, angle = 90, hjust = 1, vjust = 0.5))

PL=PL +   geom_point(aes(  x = PL$data$ID,y = PL$data$prob,
fill = ifelse(PL$data$ID %in% gen.sel,  "#377EB8","#E41A1C" )) 
,size = 2,shape = 21,stroke = 0.4) +guides(fill = "none") +  
  labs(y = "", x = "", title = "Plant Lodging")+theme(axis.text.x = element_text(size = 5, angle = 90, hjust = 1, vjust = 0.5))

GY=GY +   geom_point(aes(  x = GY$data$ID,y = GY$data$prob,
fill = ifelse(GY$data$ID %in% gen.sel,  "#377EB8","#E41A1C" )) 
,size = 2,shape = 21,stroke = 0.4) +guides(fill = "none") +  
  labs(y = "", x = "", title = "Grain Yield")+theme(axis.text.x = element_text(size = 5, angle = 90, hjust = 1, vjust = 0.5))
...
GY=GY+ theme(axis.text.x = element_text( color = ifelse(GY$data$y %in% gen.sel,  "#E41A1C", "black")))+ labs( title = "Grain Yield")
PH=PH+ theme(axis.text.x = element_text( color = ifelse(PH$data$y %in% gen.sel,  "#E41A1C", "black"))) + labs( title = "Plant Height")
PL=PL+ theme(axis.text.x = element_text( color = ifelse(PL$data$y %in% gen.sel,  "#E41A1C", "black"))) + labs( title = "Plant Lodging")

plot.pair = ggarrange(GY,PH,PL,labels = c("A","B","C"),nrow = 3, common.legend = T)
