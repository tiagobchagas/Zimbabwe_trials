...
stvc_aft <- stvc |>  select(H2, CV, trait, env, geno_sig) |> pivot_longer(H2:CV) |>  
  ggplot(aes(x=env,y=value))+geom_col(aes(fill = trait), position = position_dodge())+ facet_grid2(name~trait, scales = "free_y") + theme(legend.position = "none", axis.text.x = element_text(angle = 90)) + labs(x= "Environments", y="Parameters")+ scale_x_discrete(labels = cols.label) 
...
ggsave(plot=stvc_aft,device = "pdf",filename ="Plots/plots_parameters.pdf",dpi = "retina",height = 8,width = 10)
ggsave(plot=stvc_aft,device = "png",filename ="Plots/plots_parameters.png",dpi = "retina",height = 8,width = 10,bg = "white")
```
...
varcomp[grepl(":CROP",varcomp$effect),"type"] <- "GxY"
varcomp[grepl(":LOC",varcomp$effect),"type"] <- "GxL"
...
vc=varcomp |> ggplot(aes(x=effect,y=var, fill=type)) + geom_col(position = "stack") + theme(axis.text.x = element_text(angle=90), legend.position = "none")+ facet_wrap(~trait,scales="free");vc
