...
ggplot() + 
  ...
  #Código para definir os limites do mapa cood_sf()
    coord_sf(xlim =c(bbox_uf["xmin"],bbox_uf["xmax"]) ,ylim=c(bbox_uf["ymin"],bbox_uf["ymax"]),crs = "WGS84")+
   theme_light()
  # 
...
mapa=ggplot() + 
  ...
  geom_point(data = pontos.sf, mapping = aes(x = SITE_LONG, y = SITE_LAT),size =1,show.legend = T)+
    geom_sf_label(data = africa.sf, aes(label = ADM0_NAME),fill=NA,label.size = NA)+theme(legend.position = "none",axis.text.x =element_text(angle = 90))+labs(x="",y="");
mapa
...
mapa_pt=ggplot() + 
  ...
  #Código para plotagem do rater geom_stars()
  geom_stars(data = elev_uf.stars,alpha =0.8)+
  scale_fill_distiller(palette = "Spectral",na.value = NA,name = "Altitude")+
  geom_sf(data = uf.sf,fill=NA)+
  coord_sf(xlim =c(bbox_uf["xmin"],bbox_uf["xmax"]) ,ylim=c(bbox_uf["ymin"],bbox_uf["ymax"])) +theme_light()+
  geom_sf_label(data = brasil.sf, aes(label = UF),fill=NA,label.size = NA)+geom_point(data = pontos.sf, mapping = aes(x = longitude, y = latitude, color= regiao),size =1,show.legend = T) + 
  scale_color_manual(breaks = c("NE", "CO", "SE","SU"),values=c("#FF0000", "#0000FF","#8000FF", "#FF00FF"))+
guides(color = guide_legend(title="Região Brasileira"))+labs(y="Latitude",x="Longitude")
mapa_pt

ggsave(plot=mapa_pt,device = "pdf",filename ="Plots/common_bean_field_trials_map_pt.pdf",dpi = "retina")
ggsave(plot=mapa_pt,device = "png",filename ="Plots/common_bean_field_trials_map_pt.png",dpi = "retina")
