...
varcomp[grepl("geno",varcomp$effect),"type"] <- "G"
varcomp[grepl("CROP_SEASON",varcomp$effect),"type"] <- "Y"
varcomp[grepl("LOCATION_NAME",varcomp$effect),"type"] <- "L"
varcomp[grepl("geno:CROP",varcomp$effect),"type"] <- "GxY"
varcomp[grepl("geno:LOC",varcomp$effect),"type"] <- "GxL"
varcomp[grepl("rep",varcomp$effect),"type"] <- "R"

custom_palette=viridis::turbo(n = 19)

unique(varcomp$effect)
vc=varcomp |> ggplot(aes(x=type,y=var, fill = effect)) + geom_col(position = "stack", just = 0.5) + theme(axis.text.x = element_text(angle=90))+ facet_wrap(~trait,scales="free") +labs(x="Effects",y="Variance components",fill="Effects")+
    scale_fill_manual(
    values = custom_palette, 
    labels = c("rep"="R", "geno"="G", "LOCATION_NAME"="L","geno:LOCATION_NAME"="GxL",
 "CROP_SEASON"="Y", "geno:CROP_SEASON"="GxY", "error_env1"= "e_env1", "error_env2"="e_env2",        
  "error_env3"="e_env3","error_env4"="e_env4","error_env5"="e_env5","error_env6"="e_env6",        
"error_env7"="e_env7", "error_env8"="e_env8", "error_env9"="e_env9", "error_env10"="e_env10",
"error_env11"="e_env11","error_env12"="e_env12","error_env13"="e_env13"));vc
...
ggsave(plot=plot.perfo,device = "pdf",filename ="Plots/plots_perfo.pdf",dpi = "retina",height = 8,width = 10)
ggsave(plot=plot.perfo,device = "png",filename ="Plots/plots_perfo.png",dpi = "retina",height = 8,width = 10,bg = "white")
...
GY=a +
  theme(
          ...
          scale_fill_viridis_c(direction = 1, na.value = "white",
            limits = c(0, 1), option = "turbo") + guides(fill = guide_colorbar(barwidth = 7, barheight = 1.5, title.position = "top", title.hjust = 0.5)) + labs(title = "Grain Yield",x="")+ scale_x_discrete(labels = labels)+ scale_y_discrete(labels = labels)
...
PH=b +
  theme(
          ...
          scale_fill_viridis_c(direction = 1, na.value = "white",
            limits = c(0, 1), option = "turbo") + guides(fill = guide_colorbar(barwidth = 7, barheight = 1.5, title.position = "top", title.hjust = 0.5)) + labs(title = "Plant Heigth", y="")+ scale_x_discrete(labels = labels)+ scale_y_discrete(labels = labels)
...
```{r bpsi table}
plot.bpsi = ggarrange(a,b,labels = c("A","B"),ncol = 2, common.legend = F)
