...
library(tidyverse) 
data <- read.csv("data/Zimbabwe_rev.csv",header = T,sep = ",")
# colnames(data)
# head(data)
# data$geno <- gsub(" ", "_",data$Crop_Variety)
# data$geno <- factor(data$geno)
# # unique(pheno$IRRIGATION)
# levels(data$geno) <- paste("G", 1:length(levels(data$geno)), sep = "_")
# 
# write.csv(data,file ="data/Zimbabwe_rev.csv",row.names = FALSE)

pheno<- data[, c("CROP_SEASON","TRIAL_NAME","LOCATION_NAME","REP_NO","geno","Control",        "PL_Lodging","R8_PL_Height","GRAIN_YLD_13PCT","PLANT_DATE","HARV_DATE","SITE_LAT","SITE_LONG",       "IRRIGATION")] 
...
pheno$geno <- gsub("G_", "V00", pheno$geno ) 
factors <- c(colnames(pheno)[c(1,2,3,4,5,6,14,15,16)])
factors
str(pheno)
...
unique(pheno$TRIAL_NAME)
unique(pheno$LOCATION_NAME)
```
...
test=unique((pheno[which(pheno$Control=="Yes"),"geno"]))

table(pheno$Control,pheno$TRIAL_NAME)

# First subset the data
pheno_test <- pheno[pheno$geno %in% test, ]

# Then create the table
freq_check=table(pheno_test$geno, pheno_test$trial)
colSums(freq_check)
pheno[which(pheno$Control=="Yes"),]
pheno$year <- as.numeric(gsub(".*_", "", pheno$trial))

pheno$env <- factor(pheno$trial, 
                    levels = unique(pheno$trial[order(pheno$year, pheno$trial)]))



# pheno$env=factor(pheno$trial)

levels(pheno$TRIAL_NAME)
levels(pheno$trial)
pheno$Control
pheno$gen=factor(pheno$geno)
head(pheno$trial)
amb = model.matrix(~-1 + env, data = pheno)
ind = model.matrix(~-1 + gen, data = pheno)
amb_ind = t(amb) %*% ind
colnames(amb_ind) = sub("gen","",colnames(amb_ind))
rownames(amb_ind) = sub("env","",rownames(amb_ind))
amb_ind = ifelse(amb_ind == 0, "Absent","Present")
dim(amb_ind)

ComplexHeatmap::Heatmap(amb_ind, column_names_gp = gpar(fontsize = 5),
        row_names_gp = gpar(fontsize = 8), col = c("#EBF5E9","#00AD1A"),
        heatmap_legend_param = list(title=""), show_column_names = T,
        column_title = "Genotypes along environments")

# Create custom column labels with color highlighting
col_labels <- colnames(amb_ind)

# Example: Highlight columns containing "2018" in red
highlight_color <- ifelse(grepl("2018", col_labels), "red", "black")

ht <- ComplexHeatmap::Heatmap(
  amb_ind, 
  column_names_gp = gpar(fontsize = 5, col = highlight_color),  # Color each label differently
...
# Get the test genotypes from your pheno data
test_genos <- unique((pheno[which(pheno$Control == "Yes"), "geno"]))

# Make sure they are character, not factor
test_genos <- as.character(test_genos)

# Create highlighting vector for column labels
col_labels <- colnames(amb_ind)

# Check which columns are in test_genos
is_test_geno <- col_labels %in% test_genos

# Create heatmap with highlighted column names


pdf("Plots/Connectivity.pdf", width = 12, height = 8)  

ht <- ComplexHeatmap::Heatmap(
  amb_ind, 
  column_names_gp = gpar(
    fontsize = 5,
    col = ifelse(is_test_geno, "red", "black"),        # Red for test genotypes
    fontface = ifelse(is_test_geno, "bold", "plain")   # Bold for test genotypes
  ),
  row_names_gp = gpar(fontsize = 8), 
  col = c("#EBF5E9", "#00AD1A"),
  heatmap_legend_param = list(title = ""), 
...
for (j in names(data.list)) {
  ...
  summar.mat = matrix(nrow = length(traits), ncol = 6, 
                      dimnames = list(traits, 
                                      c("geno","error","H2", "CV","H2_em", "H2_cul")))
  ...
  for (i in traits) {
   ...
   # colocar nugget and spline
    ...
    h2 = vpredict(model.r, h2~(V1)/(V1+V2))
    h2_em <- vpredict(model.r, h2~(V1)/(V1+(V2/nrep)))
    
    sum = summary(model.r)$varcomp
    mvd = mean(((predict.asreml(model.r,classify = "geno",sed = T)$sed)^2)[
  upper.tri((predict.asreml(model.r,classify = "geno",sed = T)$sed)^2,diag=F)
])
  PEV = mean(diag((predict.asreml(model.r,classify = "geno",vcov = T)$vcov)))
  acc = sqrt(1-(PEV/sum["geno","component"]))
  h2.cul = 1-(mvd/(2*sum["geno","component"]))

  blup = predict.asreml(model.r,classify = "geno")$pvals

  gain = (mean(blup[order(blup$predicted.value, 
                              decreasing = T),]$predicted.value[1:30])-
             mean(x[,i], na.rm=T))/mean(x[,i], na.rm=T) * 100
    ...
    summar.mat[i,] = c(varcomp$signi2[1], round(varcomp$component[2],4), 
                       round(h2$Estimate,4), round(CV,4), round(h2_em$Estimate,4), round(h2.cul,4))
...
stvc$H2_em <- as.numeric(stvc$H2_em)
stvc$H2_cul <-  as.numeric(stvc$H2_cul)

stvc_bef <- stvc |>  select(H2, CV, trait, env, geno_sig,H2_em,H2_cul) |> pivot_longer(cols=c(H2,CV,H2_em,H2_cul)) |>  
...
unique(stvc |>  select(H2, CV, trait, env, geno_sig,H2_em,H2_cul) |> pivot_longer(cols=c(H2,CV,H2_em,H2_cul)))$name
...
stvc_env <- stvc |>  select(H2, CV, trait, env, geno_sig, envs,year,H2_em,H2_cul) |> pivot_longer(cols=c(CV,H2_cul)) |>  
  ...
  facet_grid2(name~trait, scales = "free_y") + theme(legend.position = "none", axis.text.x = element_text(angle = 90),
                                                     strip.text = element_text(face = "bold")) + labs(x= "Location", y="Parameters")+ scale_x_discrete(labels = cols.label);   stvc_env



stvc_year <- stvc |>  select(H2, CV, trait, env, geno_sig, envs,year,H2_em,H2_cul) |> pivot_longer(cols=c(CV,H2_cul)) |>  
...
ggsave(plot=plot.par,device = "pdf",filename ="Plots/plots_parameters_cul.pdf",dpi = "retina",height = 10,width = 12)
ggsave(plot=plot.par,device = "png",filename ="Plots/plots_parameters_cul.png",dpi = "retina",height = 10,width = 12,bg = "white")
```
...
x$L <- x$LOCATION_NAME
x$Y <- as.factor(gsub(".*_", "", pheno$trial))
x$G <- x$geno
j="G : L "
result=NULL
lrt=NULL
lrts=NULL
i <- "Plant lodging"
  lrt.mat = matrix(nrow = length(traits), ncol = 2, 
                      dimnames = list(traits, 
                                      c("L : Y", "G : L : Y")))
...
for (i in traits) {
  ...
  for (j in models) {
    ...
    str.mod[1] <- paste0("asreml::asreml(fixed=x[,i] ~ rep") 
    # str.mod[1] <- paste0("asreml::asreml(fixed=",i,"~ rep") 
    str.mod[2] <- paste0("random = ~ geno +", j)
    str.mod[3] <- "residual = ~units"
    str.mod[4] <- "data = x, maxit = 50, workspace = '1gb', na.action = na.method(x = 'include', y = 'include'))"
    
    # Combine into a complete model formula
    code <- paste(str.mod[1], str.mod[2], str.mod[3], str.mod[4], sep = ", ")
    
  
    # 
    # model.r = asreml(fixed = x[,i] ~ rep, 
    #                 random = as.formula(paste("~ geno +",j)),
    #                  maxit = 50,
    #                residual = ~units,
    #                data = x, na.action = na.method(x = "include", y = "include"))
    # model.r$call
  ...
  row = i,
  col = j,
  pvalue = lrtest$`Pr(Chisq)`,
  lrstat = lrtest$`LR-statistic`
))
       # lrt[,j]=lrtest$`LR-statistic` 
  }
    
}
    
lrtplot=results_df |> 
  ggplot(aes(x = row, y = -log(pvalue))) +
  geom_col(aes(fill=row)) +
  geom_hline(aes(yintercept = -log(0.05), 
             color = "0.05"), 
             linetype = "dashed", 
             linewidth = 1)+facet_wrap(~col)+labs(x="effect",colour="Significance",fill="Trait");lrtplot

ggsave(lrtplot,filename="Plots/lrt.pdf" ,device = "pdf",dpi = "retina")
...
unique(pheno_filt$LOCATION_NAME)
...
outs_PH$ppcheck=cbind(outs_PH$ppcheck, trait="Plant Height")
outs_PL$ppcheck=cbind(outs_PL$ppcheck, trait="Plant Lodging")
outs_GY$ppcheck=cbind(outs_GY$ppcheck, trait="Grain Yield")

ppcheck=rbind(outs_GY$ppcheck,outs_PH$ppcheck,outs_PL$ppcheck)

save(ppcheck,file = "Saves/ppcheck.rda")
ppcheck |> kableExtra::kable(format = "latex", booktabs = TRUE, longtable=TRUE)

load("Saves/ppcheck.rda")
...
varcomp <- varcomp |> mutate(effects=recode(effect,"rep"="Replication", "geno"="Variety", "LOCATION_NAME"="Location","geno:LOCATION_NAME"="Variety x Location",
 "CROP_SEASON"="Year", "geno:CROP_SEASON"="Variety x Year", "error_env1"= "E1", "error_env2"="E2",        
  "error_env3"="E3","error_env4"="E4","error_env5"="E5","error_env6"="E6",        
"error_env7"="E7", "error_env8"="E8", "error_env9"="E9", "error_env10"="E10",
"error_env11"="E11","error_env12"="E12","error_env13"="E13"))


str(varcomp)

varcomps |> kableExtra:: kable(format = "latex", booktabs = FALSE, longtable=TRUE)

varcomp[grepl("error",varcomp$effect),"type"] <- "Error"
varcomp[grepl("geno",varcomp$effect),"type"] <- "Variety"
varcomp[grepl("CROP_SEASON",varcomp$effect),"type"] <- "Year"
varcomp[grepl("LOCATION_NAME",varcomp$effect),"type"] <- "Location"
varcomp[grepl("geno:CROP",varcomp$effect),"type"] <- "Variety x Year"
varcomp[grepl("geno:LOC",varcomp$effect),"type"] <- "Variety x Location"
varcomp[grepl("rep",varcomp$effect),"type"] <- "Replication"
...
# Assuming varcomp is your data frame
# Define the desired order of levels based on the labels in scale_fill_manual
effect_levels <- c("rep", "geno", "LOCATION_NAME", "geno:LOCATION_NAME", 
                   "CROP_SEASON", "geno:CROP_SEASON", "error_env1", 
                   "error_env2", "error_env3", "error_env4", "error_env5", 
                   "error_env6", "error_env7", "error_env8", "error_env9", 
                   "error_env10", "error_env11", "error_env12", "error_env13")

# Define the corresponding labels
effect_labels <- c("Replication", "Variety", "Location", "Variety x Location", 
                   "Year", "Variety x Year", "E1", "E2", "E3", "E4", "E5", 
                   "E6", "E7", "E8", "E9", "E10", "E11", "E12", "E13")

# Convert varcomp$effect to a factor with the specified levels and labels
varcomp$effect <- factor(varcomp$effect, 
...
vc <- varcomp |> 
  ggplot(aes(x = type, y = var, fill = effect)) + 
  ...
  scale_fill_manual(
    ...
    labels = effect_labels  # Use the same labels as in the factor
  )
...
GY=GY+ theme(axis.text.y = element_text(size=5.5,),color = ifelse(GY$data$gen %in% gen.sel,  "#E41A1C", "black"),strip.text = element_text(face = "bold")) + labs( title = "Grain Yield",y="Varieties")+facet_wrap(~fac,scales="free", labeller = labeller(.cols=cols.label))+ scale_y_discrete(labels = labels) ##color = ifelse(GY$data$gen %in% gen.sel,  "#E41A1C", "black")

 PH$data  <- PH$data %>% filter(fac != "CROP_SEASON")

 
PH=PH+ theme(axis.text.y = element_text(size=5.5),strip.text = element_text(face = "bold")) + labs( title = "Plant Height",y="")+facet_wrap(~fac,scales="free", labeller = labeller(.cols=cols.label))+ 
scale_y_discrete(labels = labels)

PL=PL+ theme(axis.text.y = element_text(size=5.5),strip.text = element_text(face = "bold")) + labs( title = "Plant Lodging",y="")+facet_wrap(~fac,scales="free", labeller = labeller(.cols=cols.label))+ scale_y_discrete(labels = labels)
...
fill = ifelse(ID %in% gen.sel,  "Selected", "Not selected" )) ,size = 3,shape = 21,stroke = 0.4)+ 
  labs(y = "Probability of superior performance", x = "")+theme(axis.text.x = element_text(size = 7, angle = 90, hjust = 1, vjust = 0.5))+ scale_x_discrete(labels = labels)+
...
,size = 3,shape = 21,stroke = 0.4) +guides(fill = "none") +  
  labs(y = "", x = "Varieties")+theme(axis.text.x = element_text(size = 7, angle = 90, hjust = 1, vjust = 0.5))+ scale_x_discrete(labels = labels)+
  scale_fill_manual(  values = c("Selected" = "#E41A1C", "Not selected" ="#377EB8" ),name = "BPSI")

GY=GY +   geom_point(aes(  x = GY$data$ID,y = GY$data$prob,
fill = ifelse(GY$data$ID %in% gen.sel,  "Selected", "Not selected")) 
,size = 3,shape = 21,stroke = 0.4) +guides(fill = "none") +  
  labs(y = "", x = "")+theme(axis.text.x = element_text(size = 7, angle = 90, hjust = 1, vjust = 0.5))+ scale_x_discrete(labels = labels)+  scale_fill_manual(  values = c("Selected" = "#E41A1C", "Not selected" ="#377EB8" ),name = "BPSI")


library(ggpubr)
library(patchwork)
# plot.perfo = ggarrange(GY,PH,PL,labels = c("A","B","C"),nrow = 3, common.legend = T,hjust = c(-2.5,-2.5,-2.5) );plot.perfo
plot.perfo<- GY+PH+PL+ plot_layout(guides = 'collect',nrow = 3)+plot_annotation(tag_levels = "A") & theme(legend.position = 'top');plot.perfo


ggsave(plot=plot.perfo,device = "pdf",filename ="Plots/plots_perfo.pdf",dpi = "retina",height = 10,width = 12)
ggsave(plot=plot.perfo,device = "png",filename ="Plots/plots_perfo.png",dpi = "retina",height = 10,width = 12,bg = "white")
...
# Plot A
pa <- ggplot() + 
  geom_col(aes(x = gen, y = rank, fill = sel), data = obja |> filter(trait == "Grain Yield")) +
...
# Plot B (no y-axis)
pb <- ggplot() + 
  geom_col(aes(x = gen, y =  rank, fill = sel), data = obja |> filter(trait == "Plant Height")) +
  facet_wrap(~trait, scales = "free_x") +
  theme(
    axis.text.x = element_text(size = 4, angle = 90, hjust = 1, vjust = 0.5),
    panel.background = element_blank(),
...
# Plot C (with y-axis)
pc <- ggplot() + 
  geom_col(aes(x = gen, y =  rank, fill = sel), data = obja |> filter(trait == "Plant Lodging")) +
  facet_wrap(~trait, scales = "free_x") +
  theme(
    axis.text.x = element_text(size = 4, angle = 90, hjust = 1, vjust = 0.5),
    panel.background = element_blank(),
...
obj$trait <- "Simultaneous Selection"
# Plot D (modified to exclude fill legend contribution)
pd <- ggplot(obj, aes(x = as.factor(id), y = bpsi)) +
  geom_col(aes(fill = sel)) +
  scale_fill_manual(
    values = c("Selected" = "blue3", "Not_Selected" = "grey"),
    breaks = c("Not_Selected", "Selected"),
    labels = c("Not selected", "Selected"),
  ...
  coord_polar(start = 0, clip = "off") +  # Allow labels to extend outside plot
  geom_segment(aes(x = id, xend = id, y = bpsi, yend = max(bpsi) + 15),  # Extend segments
               color = "grey100", linewidth = 0.3) +
  geom_text(aes(x = id, y = max(bpsi) + 20, label = gen, angle = angle, hjust = hjust),  # Move labels outward
            size = 2.8) +
  facet_wrap(~trait) +  # Add facet label
  theme_minimal() +
  theme(
    ...
    strip.text = element_text(size = 8, face = "bold"),# Match facet style
    plot.margin = margin(10, 10, 10, 10)  # Add padding around plot
  ) +
  labs(fill = NULL) +
  scale_y_continuous(limits = c(NA, max(obj$bpsi) + 30), labels = NULL)

# Create the 2x2 layout with a common legend for A, B, and C
plot.bpsi <- wrap_plots(
  A = pa, 
  B = pb, 
  C = pc, 
  D = pd,
  ...
  plot_layout(guides = "collect") &  # Collect legends from pa, pb, pc
  theme(legend.position = "top")  # Position the common legend at the top

# Display the plot
print(plot.bpsi)

ggsave(plot=plot.bpsi,device = "pdf",filename ="Plots/plots_bpsi.pdf",dpi = "retina",height = 8,width = 12)
ggsave(plot=plot.bpsi,device = "png",filename ="Plots/plots_bpi.png",dpi = "retina",height = 8,width = 12, bg = "white")

```


```{r bpsi table 1}
BPSI_soy$BPSI |> kable(format = "latex", booktabs = TRUE, longtable=TRUE, row.names = F)

BPSI_soy$df |> kable(format = "latex", booktabs = TRUE, longtable=TRUE, row.names = T)
```
...
```{r bpsi table 2}
BPSI_soy$BPSI |> kable(format = "latex", booktabs = TRUE, longtable=FALSE)
