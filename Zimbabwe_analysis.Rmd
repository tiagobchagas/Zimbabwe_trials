...
for (j in names(data.list)) {
  ...
  summar.mat = matrix(nrow = length(traits), ncol = 5, 
                      dimnames = list(traits, 
                                      c("geno","error","H2", "CV","H2_em")))
  ...
  for (i in traits) {
   ...
   # colocar nugget and spline
    ...
    summar.mat[i,] = c(varcomp$signi2[1], round(varcomp$component[2],4), 
                       round(h2$Estimate,4), round(CV,4), round(h2_em$Estimate,4))
...
stvc_bef <- stvc |>  select(H2, CV, trait, env, geno_sig,H2_em) |> pivot_longer(cols=c(H2,CV,H2_em)) |>  
...
unique(stvc |>  select(H2, CV, trait, env, geno_sig,H2_em) |> pivot_longer(cols=c(H2,CV,H2_em)))$name

stvc$year <- gsub(".*_", "", stvc$env)
stvc$envs <- gsub("_.*", "", stvc$env)
head(stvc)

stvc_env <- stvc |>  select(H2, CV, trait, env, geno_sig, envs,year,H2_em) |> pivot_longer(cols=c(H2,CV,H2_em)) |>  
...
stvc_year <- stvc |>  select(H2, CV, trait, env, geno_sig, envs,year,H2_em) |> pivot_longer(cols=c(H2,CV,H2_em)) |>  
