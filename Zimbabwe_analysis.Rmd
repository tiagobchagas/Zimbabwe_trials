...
GY=GY+ theme(axis.text.y = element_text(size=5.5,color = ifelse(GY$data$gen %in% gen.sel,  "#E41A1C", "black")),strip.text = element_text(face = "bold")) + labs( title = "Grain Yield",y="Varieties")+facet_wrap(~fac,scales="free", labeller = labeller(.cols=cols.label))+ scale_y_discrete(labels = labels)

 PH$data  <- PH$data %>% filter(fac != "CROP_SEASON")

 
PH=PH+ theme(axis.text.y = element_text(size=5.5, color = ifelse(PH$data$gen %in% gen.sel,  "#E41A1C", "black")),strip.text = element_text(face = "bold")) + labs( title = "Plant Height",y="")+facet_wrap(~fac,scales="free", labeller = labeller(.cols=cols.label))+ 
...
fill = ifelse(ID %in% gen.sel,  "Selected", "Not selected" )) ,size = 3,shape = 21,stroke = 0.4)+ 
  labs(y = "Probability of superior performance", x = "", title = "Plant Height")+theme(axis.text.x = element_text(size = 7, angle = 90, hjust = 1, vjust = 0.5))+ scale_x_discrete(labels = labels)+
...
,size = 3,shape = 21,stroke = 0.4) +guides(fill = "none") +  
  labs(y = "", x = "Varieties", title = "Plant Lodging")+theme(axis.text.x = element_text(size = 7, angle = 90, hjust = 1, vjust = 0.5))+ scale_x_discrete(labels = labels)+
  scale_fill_manual(  values = c("Selected" = "#E41A1C", "Not selected" ="#377EB8" ),name = "BPSI")

GY=GY +   geom_point(aes(  x = GY$data$ID,y = GY$data$prob,
fill = ifelse(GY$data$ID %in% gen.sel,  "Selected", "Not selected")) 
,size = 3,shape = 21,stroke = 0.4) +guides(fill = "none") +  
  labs(y = "", x = "", title = "Grain Yield")+theme(axis.text.x = element_text(size = 7, angle = 90, hjust = 1, vjust = 0.5))+ scale_x_discrete(labels = labels)+  scale_fill_manual(  values = c("Selected" = "#E41A1C", "Not selected" ="#377EB8" ),name = "BPSI")
...
# Plot A
pa <- ggplot() + 
  geom_col(aes(x = gen, y = max(rank) - rank, fill = sel), data = obja |> filter(trait == "Grain Yield")) +
...
# Plot B (no y-axis)
pb <- ggplot() + 
  geom_col(aes(x = gen, y = max(rank) - rank, fill = sel), data = obja |> filter(trait == "Plant Height")) +
  facet_wrap(~trait, scales = "free_x") +
  theme(
    axis.text.x = element_text(size = 4, angle = 90, hjust = 1, vjust = 0.5),
    panel.background = element_blank(),
...
# Plot C (with y-axis)
pc <- ggplot() + 
  geom_col(aes(x = gen, y = max(rank) - rank, fill = sel), data = obja |> filter(trait == "Plant Lodging")) +
  facet_wrap(~trait, scales = "free_x") +
  theme(
    axis.text.x = element_text(size = 4, angle = 90, hjust = 1, vjust = 0.5),
    panel.background = element_blank(),
...
pd <- ggplot(obj, aes(x = as.factor(id), y = max(bpsi) - bpsi)) +
  ...
  geom_segment(aes(x = id, xend = id, y = max(bpsi) - bpsi, yend = max(bpsi) + 15),  # Extend segments
...
# Show the layout to identify which columns contain each facet
gtable_show_layout(g1)

## Create individual facet plots by extracting the complete panel areas
# First facet: columns 6-8, rows 8-11
g1_facet1 <- g1[8:11, 6:8]
g1_facet1 <- gtable_add_grob(g1_facet1, g1$grobs[[17]], t = 1, l = 2)  # Add strip

# Second facet: columns 10-12, rows 8-11
g1_facet2 <- g1[8:11, 10:12]
g1_facet2 <- gtable_add_grob(g1_facet2, g1$grobs[[18]], t = 1, l = 2)  # Add strip

# Third facet: columns 14-16, rows 8-11
g1_facet3 <- g1[8:11, 14:16]
g1_facet3 <- gtable_add_grob(g1_facet3, g1$grobs[[19]], t = 1, l = 2)  # Add strip

# Display the 2x2 grid
# grid.newpage()
# grid.arrange(
#   g1_facet1, g1_facet2, g1_facet3, b,
#   nrow = 2,
#   ncol = 2
# )
# 
...
# library(gtable)
# 
# # First, ensure each facet has proper margins for separation
# # Add padding to each facet grob
# add_padding <- function(grob, padding = unit(10, "pt")) {
#   # Create a padding grob
#   padded_grob <- gtable_add_padding(grob, padding)
#   return(padded_grob)
# }
# 
# # Add padding to each facet
# g1_facet1_padded <- add_padding(g1_facet1)
# g1_facet2_padded <- add_padding(g1_facet2)
# g1_facet3_padded <- add_padding(g1_facet3)
# b_padded <- add_padding(b)
# 
# # Create the 2x2 grid with proper spacing
# grid.newpage()
# grid.arrange(
#   g1_facet1_padded,    # Top left (A)
#   g1_facet2_padded,    # Top right (B)
#   g1_facet3_padded,    # Bottom left (C)
#   b,            # Bottom right (D)
#   nrow = 2,
#   ncol = 2,
#   padding = unit(20, "pt")  # Space between plots
# )
# 
# # Manually add annotation letters
# grid.text("A", x = 0.01, y = 0.98, gp = gpar(fontsize = 16, fontface = "bold"))
# grid.text("B", x = 0.50, y = 0.98, gp = gpar(fontsize = 16, fontface = "bold"))
# grid.text("C", x = 0.01, y = 0.50, gp = gpar(fontsize = 16, fontface = "bold"))
# grid.text("D", x = 0.50, y = 0.50, gp = gpar(fontsize = 16, fontface = "bold"))


# plot.bpsi <- wrap_plots(plot1,plot2,plot3, b, ncol =2, heights = c(2, 1)) +
#   plot_annotation(tag_levels = "A")   # Right margin for width
# 
# 
# plot.bpsi <- a / b  + plot_layout(ncol = 1,heights = c(1, 2) )
# 

 wrap1 <- wrap_elements(g1_facet1)
 wrap2 <- wrap_elements(g1_facet2)
 wrap3 <- wrap_elements(g1_facet3)
